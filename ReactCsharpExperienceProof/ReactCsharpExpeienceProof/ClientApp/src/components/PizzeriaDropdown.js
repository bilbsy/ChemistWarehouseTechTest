...
function PizzeriaDropdown(props) {
    ...
    const addNewPizzeria = () => {
        if (showAddPizzeria) {
            ...
            fetch('pizzeria?' + new URLSearchParams({
                name: newPizzeria.name,
                location: newPizzeria.location
            }), requestOptions)
                        ...
                        <InputGroup className="justify-content-md-center">
                            <DropdownButton title={selectedPizzeria.name ? selectedPizzeria.name : "Select an item"}>
                                {pizzerias.map((item, index) => (
                                    <Dropdown.Item key={index} onClick={() => setSelectedPizzeria((e) => ({
                                        ...e,
                                        id: item.id,
                                        name: item.name
                                    }))}>
                            ...
                            </DropdownButton>
                            {
                                showAddPizzeria ?
                                    <>
                                        <Input value={newPizzeria['name']}
                                               onChange={e => handleNewPizzeriaOnChange('name', e.target.value)}
                                               placeholder="Enter pizzeria name"></Input>
                                        <Input value={newPizzeria['location']}
                                               onChange={e => handleNewPizzeriaOnChange('location', e.target.value)}
                                               placeholder="Enter pizzeria location"></Input>
                                        <Button variant="danger" onClick={() => {
                                            hideNewPizzeria()
                                        }}><RiCloseFill icon="fa-light fa-xmark"/></Button>
                                    </>
                                    : null
                            }
                            <Button variant="success" onClick={() => {
                                addNewPizzeria()
                            }}><FaPlus icon="fa-light fa-plus"/></Button>
                        </InputGroup>
                    </Col>
                </Row>
